...
public:
 ...
 void initialize(UAS &uas_) override
 {
	 ...
	 fp_nh.param("use_gazebo_states", use_gazebo_states, false);
	 ...
	 else if (use_gazebo_states) {	// Gazebo data in PoseStamped msg
		 gazebo_sub_ = fp_nh.subscribe("/gazebo/model_states", 1, &FakeGPSPlugin::mocap_pose_cb, this);
	 }
...
private:
 ...
 bool use_gazebo_states;		//!< set use of gazebo states
 ...
 void gazebo_cb(const gazebo_msgs::ModelStates::ConstPtr& msg) {
	 ...
	 // Find the index of the 'drone' model
	 for (size_t i = 0; i < model_names.size(); ++i) {
		 if (model_names[i] == "drone") {
			 drone_index = i;
			 break;
		 }
	 }
	 ...
	 if (drone_index == -1) {
		 ROS_ERROR("Model 'drone' not found in model_states!");
		 return;
	 }

	 // Obtain pose of drone and update current position
	 Eigen::Affine3d pos_enu;
	 tf::poseMsgToEigen(msg->pose[drone_index], pos_enu);
	 send_fake_gps(ros::Time::now(), ftf::transform_frame_enu_ecef(Eigen::Vector3d(pos_enu.translation()), map_origin));
